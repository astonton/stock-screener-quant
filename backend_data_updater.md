...
def calculate_simple_piotroski(ticker_obj):
    """Menghitung aproksimasi Piotroski F-Score (Skala 0-6)."""
    score = 0
    try:
...
def calculate_simple_dcf(ticker_obj, current_price, discount_rate=0.10, growth_rate=0.05, terminal_rate=0.02, years=5):
    """
    Menghitung estimasi Nilai Wajar (Fair Value) menggunakan DCF sederhana.
    Asumsi: discount_rate (WACC proxy): 10%, growth_rate: 5%, terminal_rate: 2%
    """
    try:
        cf = ticker_obj.cashflow
    ...
    for ticker_symbol, region in WATCHLIST.items():
        try:
            ...
            if hist.empty or len(hist) < 200:
                print(f"Data histori tidak cukup untuk {ticker_symbol}")
                continue

...
col2.metric("Saham Undervalued (Diskon)", len(filtered_df[filtered_df["Status Valuasi"] == "Undervalued (Diskon)"]))
...
# Fungsi untuk mewarnai baris tabel berdasarkan Margin of Safety
def color_mos(val):
...
st.dataframe(
    filtered_df.style.applymap(color_mos, subset=['MoS (%)']),
...
if not plot_val_df.empty:
    ...
    st.info("Data Margin of Safety tidak tersedia (mungkin karena FCF negatif).")

st.caption("*Catatan: Perhitungan DCF menggunakan asumsi sederhana (WACC 10%, Pertumbuhan FCF 5%, Terminal Rate 2%).*")